...
>
        ...
        <div className="hidden md:flex gap-8">
          {navLinks.map((link) => (
            <a
              key={link.label}
              href={link.href}
              className="text-slate-200 hover:text-cyan-300 transition-colors duration-200 text-sm font-medium relative group"
            >
              {link.label}
      ...
      {/* Mobile Menu */}
      {mobileMenuOpen && (
        <div className="md:hidden bg-slate-900/95 border-t border-slate-700 backdrop-blur-md animate-slideDown">
          <div className="px-6 py-4 space-y-4">
            {navLinks.map((link) => (
              <a
                key={link.label}
                href={link.href}
                className="block text-slate-200 hover:text-cyan-300 transition-colors py-2"
            ...
            <button 
              onClick={handleLetsTalkClick}
              className="w-full px-4 py-2 bg-gradient-to-r from-cyan-500 to-blue-500 text-white rounded-lg hover:shadow-lg hover:shadow-cyan-500/50 transition-all duration-200 font-medium"
            >
              Let's Talk
            </button>
          </div>
